<script>
    ...
    function showlist() {
        ...
        var existingTable = $('#country-table').DataTable();
        if (existingTable) {
            existingTable.destroy();
        }
        new DataTable('#country-table', {
            processing: true,
            searching: true,
            ajax: {
                url: "{{ route('get-manage-country.laksh_country') }}",
                method: 'POST',
                headers: {
    ...
    function Status_es_Change(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('change-country-status.laksh_country') }}",
                    data: {
                        id: id,
    ...
    function DeleteData(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('delete-country-data.laksh_country') }}",
                    data: {
                        id: id
                    },
                    headers: {
    ...
    function CreateData() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('create-the-country.laksh_country') }}",
            data: formdata,
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
    ...
    function EditData(id) {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('get-the-country.laksh_country') }}",
            data: {
                id: id
            },
            headers: {
            ...
            success: function(response) {
                ...
                if (response['status'] == 200) {
                    $('#edit_country_title').val(response['data'].name)
                    $('#edit_country_code').val(response['data'].code)
                    $('#edit_mobile_code').val(response['data'].phonecode)
    ...
    function UpdateTheData() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('update-the-country.laksh_country') }}",
            data: formdata,
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
