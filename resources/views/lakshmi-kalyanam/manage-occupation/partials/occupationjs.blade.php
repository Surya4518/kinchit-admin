<script>
    ...
    function showlist() {
        ...
        var existingTable = $('#occupation-table').DataTable();
        if (existingTable) {
            existingTable.destroy();
        }
        new DataTable('#occupation-table', {
            processing: true,
            searching: true,
            ajax: {
                url: "{{ route('get-manage-occupation.laksh_occupation') }}",
                method: 'POST',
                headers: {
    ...
    function Status_es_Change(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('change-occupation-status.laksh_occupation') }}",
                    data: {
                        id: id,
                        ...
                        if (response['status'] == 200) {
                            Swal.fire({
                                ...
                                timer: 2500
                                  ...
                                  var table = $('#occupation-table').DataTable();
    ...
    function DeleteData(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('delete-occupation-data.laksh_occupation') }}",
                    data: {
                        id: id
                    },
                    headers: {
    ...
    function CreateData() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('create-the-occupation.laksh_occupation') }}",
            data: formdata,
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
            success: function(response) {
                if (response['status'] == 200) {
                    ...
                    $('#create_occupation_modal').modal('hide')
                    Swal.fire({
                        position: "top-right",
                        icon: "success",
                        title: 'Success..!',
    ...
    function EditData(id) {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('get-the-occupation.laksh_occupation') }}",
            data: {
                id: id
            },
            headers: {
    ...
    function UpdateTheData() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('update-the-occupation.laksh_occupation') }}",
            data: formdata,
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
            success: function(response) {
                if (response['status'] == 200) {
                    $("#data_update_form")[0].reset();
                    $('#edit_occupation_modal').modal('hide')
                    Swal.fire({
                        position: "top-right",
                        icon: "success",
                        title: 'Success..!',
