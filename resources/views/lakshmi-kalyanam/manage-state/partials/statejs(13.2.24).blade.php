<script>
    $(function() {
        initializeSelect2InsideModal("#create_state_modal", "#country");
        initializeSelect2InsideModal("#edit_state_modal", "#edit_country");
    ...
    function showlist() {
        ...
        var existingTable = $('#state-table').DataTable();
        if (existingTable) {
        ...
        new DataTable('#state-table', {
            ...
            searching: true,
            ajax: {
                url: "{{ route('get-manage-state.laksh_state') }}",
                method: 'POST',
                headers: {
    ...
    function Status_es_Change(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('change-state-status.laksh_state') }}",
                    data: {
                        id: id,
    ...
    function DeleteData(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('delete-state-data.laksh_state') }}",
                    data: {
                        id: id
                    },
                    headers: {
    ...
    function CreateData() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('create-the-state.laksh_state') }}",
            data: formdata,
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
    ...
    function EditData(id) {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('get-the-state.laksh_state') }}",
            data: {
                id: id
            },
            headers: {
            ...
            success: function(response) {
                ...
                if (response['status'] == 200) {
                    $('#edit_state_title').val(response['data'].name)
    ...
    function UpdateTheData() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('update-the-state.laksh_state') }}",
            data: formdata,
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
