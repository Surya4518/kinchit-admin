<script>
    $(function() {
        initializeSelect2InsideModal("#create_city_modal", "#state");
        initializeSelect2InsideModal("#edit_city_modal", "#edit_state");
    ...
    function showlist() {
        ...
        var existingTable = $('#city-table').DataTable();
        if (existingTable) {
            existingTable.destroy();
        }
        new DataTable('#city-table', {
            processing: true,
            searching: true,
            ajax: {
                url: "{{ route('get-manage-city.laksh_city') }}",
                method: 'POST',
                headers: {
    ...
    function Status_es_Change(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('change-city-status.laksh_city') }}",
                    data: {
                        id: id,
    ...
    function DeleteData(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('delete-city-data.laksh_city') }}",
                    data: {
                        id: id
                    },
                    headers: {
    ...
    function CreateData() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('create-the-city.laksh_city') }}",
            data: formdata,
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
    ...
    function EditData(id) {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('get-the-city.laksh_city') }}",
            data: {
                id: id
            },
            headers: {
            ...
            success: function(response) {
                ...
                if (response['status'] == 200) {
                    $('#edit_city_title').val(response['data'].name)
    ...
    function UpdateTheData() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('update-the-city.laksh_city') }}",
            data: formdata,
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
