<script>
    ...
    function showlist() {
        ...
        new DataTable('#category-table', {
            ...
            searching: true,
            ajax: {
                url: "{{ route('show-category-list.tut_category') }}",
                method: 'POST',
                headers: {
    ...
    function CreateTutorialCategory() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('create-tutorial-category.tut_category') }}",
            data: formdata,
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
    ...
    function EditTutorialCategory(id) {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('get-tutorial-category-data.tut_category') }}",
            data: {
                id: id
            },
            headers: {
    ...
    function UpdateTheDataTutorialCategory() {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('update-tutorial-category-data.tut_category') }}",
            data: formdata,
            headers: {
                'X-CSRF-TOKEN': csrfToken
            },
    ...
    function DeleteTheTutorialCategory(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('delete-tutorial-category-data.tut_category') }}",
                    data: {
                        id: id
                    },
                    headers: {
    ...
    function UpdateSortOrder(id) {
        ...
        $.ajax({
            type: 'post',
            url: "{{ route('update-tutorial-category-sortorder.tut_category') }}",
            data: {
                id: id,
    ...
    // // Iterate through each row and update the S.No column
    // table.rows().every(function (rowIdx, tableLoop, rowLoop) {
    //     var data = this.data();
    //     data['#'] = rowIdx + 1; // Update the S.No column
    //     table.row(rowIdx).data(data); // Update the row data
    // });
