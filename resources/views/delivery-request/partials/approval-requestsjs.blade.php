<script>
    ...
    function showlist(id) {
        ...
        new DataTable('#approved-table', {
            ...
            searching: true,
            ajax: {
                url: "{{ route('show-delivery-requests.index') }}",
                method: 'POST',
                data: {
                    id: id
                },
                    ...
                    render: function(data, type, row) {
                        ...
                        return `<ul style="display: flex;">
                         ...
                         <li  style="list-style-type: none;margin-left: 10px;""><a href="javascript:Request_reject_modal(${row.id});" title="Reject It" class="mb-2 mr-2 btn-sm btn-transition btn btn-outline-secondary is_reject"><i class="fas fa-times"></i></a></li>
                          <a href="/delivery-approve-request-view/${row.id}" style="list-style-type: none;margin-left: 10px;"" title="View" class="mb-2 mr-2 btn-sm btn-transition btn btn-outline-secondary"><i class="fas fa-eye"></i></a>
                         </ul>`;
    ...
    function Request_modal(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "delivery-approve-status/" + id,
                    data: {
                        id: id,
                    ...
                    success: function(response) {
                        if (response.status == 200) {
                            ...
                            window.location.href = '/delivery-pending-requests';
                        }
                    },
                    error: function(data) {
    ...
    function Request_reject_modal(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "delivery-rejected-status/" + id,
                    data: {
                        id: id,
