...
<head>
    ...
    <title> KINCHITKARAM :: Replies </title>
...
<body class="main-body app sidebar-mini ltr" oncontextmenu="return false;">
            ...
            <div class="main-container container-fluid">
                ...
                <div class="breadcrumb-header justify-content-between">
                    <div>
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="javascript:void(0);">Replies</a></li>
                                <li class="breadcrumb-item active" aria-current="page"> Edit Reply
                                </li>
                            </ol>
                        </nav>
                    </div>
                ...
                <div class="card">
                    <div class="row">
                        ...
                        <div class="card-body">
                           <div class="row">
                            <form method="post" id="update_reply_frm">
                                <div class="row">
                                    <div class="col-6">
                                        <label for="reply_title">Threads</label>
                                        <select class="form-control" name="reply_parent_thread_id" id="reply_parent_thread_id">
                                            <option value="">Select Thread</option>
                                            @for ($i=0; $i < $threads->count(); $i++)
                                            <option {{ $replies[0]->post_parent == $threads[$i]->ID ? 'selected' : '' }} value="{{ $threads[$i]->ID }}">{{ $threads[$i]->post_title }}</option>
                                            @endfor
                                        </select>
                                        <p class="validate_errors" id="reply_parent_thread_id_error" style="color: #f90404;font-size: 13px;font-family: 'circular';"></p>
                                    </div>
                                    <div class="col-6">
                                        <label for="reply_title">Reply Title</label>
                                        <input type="text" class="form-control" name="reply_title" id="reply_title" value="{{ $replies[0]->post_title }}" placeholder="Enter Reply Title">
                                        <p class="validate_errors" id="reply_title_error" style="color: #f90404;font-size: 13px;font-family: 'circular';"></p>
                                    </div>
                                    <div class="col-6">
                                        <label for="reply_post_status">Publish Status</label>
                                        <select class="form-control" name="reply_post_status" id="reply_post_status">
                                            <option value="">Select Status</option>
                                            <option {{ $replies[0]->post_status == 'publish' ? 'selected' : '' }} value="publish">Public</option>
                                            <option {{ $replies[0]->post_status == 'pending' ? 'selected' : '' }} value="pending">Pending</option>
                                        </select>
                                        <p class="validate_errors"  id="reply_post_status_error" style="color: #f90404;font-size: 13px;font-family: 'circular';"></p>
                                    </div>
                                </div>
                                <div class="row mt-4">
                                    <div class="col-12">
                                        <label for="reply_content">Reply</label>
                                        <textarea class="summernote form-control" name="reply_content" id="reply_content" cols="30" rows="10">{{ $replies[0]->post_content }}</textarea>
                                        <p class="validate_errors"  id="reply_content_error" style="color: #f90404;font-size: 13px;font-family: 'circular';"></p>
                                    </div>
                                    <div class="col-12 text-center mg-t-40">
                                        <input type="hidden" name="edit_reply_id" id="edit_reply_id" value="{{ $replies[0]->ID }}">
                                        <button type="button" class="btn btn-secondary" id="update-reply">Submit</button>
                                    </div>
                                </div>
                            </form>
                           </div>
    ...
    <script></script>
    @include('dharma-sandheha.replies.partials.repliesjs')
