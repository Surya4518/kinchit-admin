...
class ManageStateController extends Controller
{
    public function index(Request $request)
    {
        ...
        $country = ManageCountry::whereNull('deleted_at')
            ->where('status', '=', 'Active')
            ->orderBy('id', 'ASC')
            ->get();
        return view('lakshmi-kalyanam.manage-state.index', compact('country', 'alldatas'));
    }

    public function ShowList(Request $request)
    {
        $result = ManageState::from('state')
    ...
    {
        $model = ManageState::find($request->id);
        $update = $model->update([
    ...
    {
        $delete = ManageState::find($request->id)->delete();
    ...
    public function CreateData(Request $request)
    {
        ...
        $create = ManageState::create([
            'name' => $request->state_title,
    ...
    {
        $result = ManageState::find($request->id);
    ...
    public function UpdateData(Request $request)
    {
        ...
        $update = ManageState::find($request->edit_id)->update([
            'name' => $request->edit_state_title,
            'country' => $request->edit_country,
            'sortorder' => $request->edit_sort_order,
            'status' => $request->edit_status_of_data,
        ]);
        return response()->json($update == true ? ['status' => 200, 'message' => 'Data has been successfully updated.'] : ['status' => 400, 'message' => 'Failed to update data.']);
    }
}
